...
aes_rval ccaes_gladman_encrypt_key(const uint8_t *key, size_t key_len, ccaes_gladman_encrypt_ctx cx[1])
{
    switch(key_len)
    {
#if defined( AES_ERR_CHK )
...
aes_rval ccaes_gladman_decrypt_key(const unsigned char *key, int key_len, ccaes_gladman_decrypt_ctx cx[1])
{
    switch(key_len)
    {
#if defined( AES_ERR_CHK )
