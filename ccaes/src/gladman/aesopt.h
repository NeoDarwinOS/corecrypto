...
or exclude it from compilation.
 ...
 IMPORTANT NOTE: If you are using this C interface with dynamic tables make sure
that you call genTabs() before AES is used so that the tables are initialised.
...
/*	Select which AES implementation to use.  Preprocessor directives decide
        ...
        "GladmanAES" to "AESGladman", since this groups directories by algorithm
   in listings, but I am resisting for the moment.)
*/
...
/*  11. TABLE POINTER CACHING
    ...
    cache pointers to the tables in the encrypt/decrypt code.  Note that this
        only works if you are using FOUR_TABLES for the ROUND you enable this
   for.
*/
#if 1
...
#define upr(x, n)                                                              \
  (((aes_32t)(x) << (8 * (n))) | ((aes_32t)(x) >> (32 - 8 * (n))))
...
#define word_out(x, c, v)                                                      \
  {                                                                            \
    ((aes_08t *)(x) + 4 * c)[0] = bval(v, 0);                                  \
    ((aes_08t *)(x) + 4 * c)[1] = bval(v, 1);                                  \
    ((aes_08t *)(x) + 4 * c)[2] = bval(v, 2);                                  \
    ((aes_08t *)(x) + 4 * c)[3] = bval(v, 3);                                  \
  }
...
#define gf_mulx(x) (u = (x) & m1, u |= (u >> 1), ((x) & m2) << 1) ^ ((u >> 3) |
(u >> 6)) #define m4  (0x01010101 * BPOLY) #define gf_mulx(x) (u = (x) & m1,
((x) & m2) << 1) ^ ((u - (u >> 7)) & m4)
*/
