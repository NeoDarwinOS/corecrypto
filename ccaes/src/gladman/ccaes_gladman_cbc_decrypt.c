...
cc_error_t ccaes_gladman_cbc_decrypt_init(const struct ccmode_cbc *cbc,
                                          cccbc_ctx *ctx,
                                          size_t key_size,
                                          const void *key)
{
...
cc_error_t ccaes_gladman_cbc_decrypt(const cccbc_ctx *ctx,
                                     cccbc_iv *iv,
                                     size_t nblocks,
                                     const void *in,
                                     void *out)
{
    return ccaes_gladman_decrypt(ctx, iv, nblocks, (const uint8_t *)in, (uint8_t *)out);
}

const struct ccmode_cbc ccaes_gladman_cbc_decrypt_mode = {
    .size = ccn_sizeof_size(sizeof(ccaes_gladman_decrypt_ctx)),
