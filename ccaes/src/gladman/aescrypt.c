...
cc_error_t ccaes_gladman_encrypt(const cccbc_ctx *ctx, cccbc_iv *in_iv, size_t num_blk,
                                 const uint8_t *in, uint8_t *out)
{
    const ccaes_gladman_encrypt_ctx *cx = (const ccaes_gladman_encrypt_ctx *)ctx;
...
cc_error_t ccaes_gladman_decrypt(const cccbc_ctx *ctx, cccbc_iv *in_iv, size_t num_blk,
                                 const uint8_t *in, uint8_t *out)
{
    const ccaes_gladman_decrypt_ctx *cx = (const ccaes_gladman_decrypt_ctx *)ctx;
    aes_32t        locals(b0, b1);
	...
	if(cbcEnable & (num_blk != 0)) {
		cc_memmove(lastIv, in, AES_BLOCK_SIZE);
	}
	...
	if(cbcEnable) {
		cc_memmove(in_iv, lastIv, AES_BLOCK_SIZE);
	}
