...
cc_error_t ccblowfish_ltc_setup(const struct ccmode_ecb *ecb,
                                ...
                                const void *key)
{
    ltc_blowfish_key *skey = (ltc_blowfish_key *)ctx;
    uint32_t x, y, z, A;
    uint8_t B[8];
    
    cc_debug_client_abort((key_size < 8 || key_size > 56), "ccblowfish_ltc_setup: Bad key size.", CCERR_PARAMETER);
    
    /* load in key bytes (Supplied by David Hopwood) */
    for (x = y = 0; x < 18; x++) {
        A = 0;
        for (z = 0; z < 4; z++) {
            A = (A << 8) | ((uint32_t)((const uint8_t *)key)[y++] & 255);
            if (y == (uint32_t)key_size) {
                y = 0;
            }
        }
        skey->K[x] = ORIG_P[x] ^ A;
    ...
    for (x = 0; x < 18; x += 2) {
        /* encrypt it */
        ccblowfish_ltc_ecb_encrypt(ctx, 1, B, B);
        /* copy it */
        cc_load32_be(skey->K[x], &B[0]);
        cc_load32_be(skey->K[x+1], &B[4]);
    }
    ...
    for (x = 0; x < 4; x++) {
        for (y = 0; y < 256; y += 2) {
            ...
            ccblowfish_ltc_ecb_encrypt(ctx, 1, B, B);
            /* copy it */
            cc_load32_be(skey->S[x][y], &B[0]);
            cc_load32_be(skey->S[x][y+1], &B[4]);
        }
    }
